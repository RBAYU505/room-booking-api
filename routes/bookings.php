...
function handleBookings($path, $method)
{
    ...
    if ($endpoint == "/bookings" && $mtd === null && $prm === null && $method == "POST") { //CREATE BOOKING
        ...
        $check = pg_query_params($db, "SELECT * FROM bookings WHERE id = $1", [$id]);
        ...
        pg_query_params($db, "DELETE FROM bookings WHERE id = $1", [$id]);
        ...
        $uid = $prm;
        $res = pg_query_params($db, "SELECT b.*, r.name as ruangan FROM bookings b JOIN rooms r ON r.id=b.room_id WHERE b.user_id = $1 ORDER BY b.start_time", [$uid]);
        $bookings = [];
        while ($b = pg_fetch_assoc($res)) {
            $bookings[] = $b;
        }
        ...
        $res = pg_query($db, "SELECT b.*, r.name as ruangan FROM bookings b JOIN rooms r ON r.id=b.room_id ORDER BY b.start_time");
        $bookings = [];
        while ($b = pg_fetch_assoc($res)) {
            $bookings[] = $b;
        }
